...
#define S5L8702_OTGPHY_CON 0x44
...
{
    /* Adjusted for S5L8930. Based on the iDroid/openiBoot code. */
	struct s5l8702_usbphy *usbphy = phy_get_drvdata(phy);
	u32 pwr;
    printk("%s...\n", __func__);
	pwr = readl(usbphy->base);
	pwr &= ~0x19;
	writel_relaxed(pwr, usbphy->base + S5L8702_OTGPHY_PWR);
    mdelay(10);
	writel_relaxed(6, usbphy->base + S5L8702_OTGPHY_UNKCON);
	writel_relaxed(1843, usbphy->base + S5L8702_OTGPHY_CON);
    mdelay(10);
